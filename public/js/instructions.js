function checkAnswers(){
    ...
    'represent','undo','attentionSort','attentionCorrect'];
    ...
    attentionCorrectCorrect = false;
    attentionCorrect = document.getElementsByName('attentionCorrect');
    for(i=0; i < attentionCorrect.length; i++){
        if (attentionCorrect[i].type == 'radio' && attentionCorrect[i].checked == true){
            if (attentionCorrect[i].value == 1){
                attentionCorrectCorrect = true;
                attentionCorrect[i].classList.add('is-valid');
            }else{
                attentionCorrect[i].classList.add('is-invalid');
                attentionCorrectCorrect = false;
            }
        }else{
            attentionCorrect[i].classList.remove('is-invalid')
            attentionCorrect[i].classList.remove('is-valid');
        }
    }

    if(attentionCorrectCorrect && attentionSortCorrect && undoCorrect && representCorrect && considerCorrect){
