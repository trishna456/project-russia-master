...
function dragElement(elmnt) {
	...
	var obj_SC = document.getElementById("SomewhatComfortable");
	var obj_NC = document.getElementById("NotComfortable");
	var obj_C = document.getElementById("Comfortable");
	var obj_VC = document.getElementById("VeryComfortable");
	...
	function closeDragElement() {
		...
		if(obj != null){
			...
			var SortedInto = stack.slice(0,stack.indexOf("Comfortable"))
			SortedInto = SortedInto +"Comfortable"
	...
	function cardOverObj(top, left) {
		...
		if ( checkCollision(rect_card,rect_SC) ){
			...
			addToPath("Somewhat Comfortable");
		}else{
			obj_SC.style.backgroundColor = ""
			obj_SC.style.color = "Black"
		}
...
function getCard(){
	$.get(origin+'/IoTCardSorting/Card', {SortingType:'Comfort', Previous:'0'}, getCardCallback);
}

function saveCardSort(datatype, stackName){
	$.post(origin+"/IoTCardSorting/Card",
	{
		CardName: datatype,
...
function saveCardSortCallback(data, status){
	if(status == "success"){
		var timeTaken = (parseInt(data.comfortEndTime) - parseInt(data.comfortStartTime))/60000; 
		dashboard.innerHTML  = "The card <b style:'color: black'>"+ data.name + "</b> was placed in <b style:'color: black'>" + data.comfortStackName +"!!</b>"; 
...
function getCardCallback(data, status){
	if(status == "success"){
		...
		if (card.name != "Done"){
			features = shuffle(card.features);

			var featurelist = "";
			var featureDecHtml = "<p class='list-group-item list-group-item-dark' style='width: 20rem;'><b> Feature Descriptions </b></p>";
			var numOfFeatures = card.features.length;
			for(i=0; i < numOfFeatures; i++){
				featurelist = featurelist + "<li class='list-group-item'>" + card.features[i].name +"</li>";
				featureDecHtml = featureDecHtml + "<p class='list-group-item list-group-item-secondary col-12'><b>"+ card.features[i].name +":</b> " +card.features[i].description+"</p>"
			}

			card_name.src = card.name;

			card_description.innerHTML = card.description;
			card_features.innerHTML = featurelist;
...
function getPreviousCard(){
	$.get(origin+'/IoTCardSorting/Card', {SortingType:'Comfort', Previous:'1'}, getPreviousCallback);
}
...
function getPreviousCallback(data, status){
	...
	if(data != null && status == "success"){
		var card = data.card;
		var timeTaken = (parseInt(card.comfortEndTime) - parseInt(card.comfortStartTime))/60000; 
		dashboard.innerHTML  = "The card <b style:'color: black'>"+ card.name + "</b> was placed in <b style:'color: black'>" + card.comfortStackName +"!!</b>"; 
...
function undoSort(){
	$.post(origin+"/IoTCardSorting/Undo",{SortingType:'Comfort'}, undoCallback);
}
...
function undoCallback(data, status){
	...
	if( status == "success"){
		var featurelist = "";
		var featureDecHtml = "<p class='list-group-item list-group-item-dark' style='width: 20rem;'><b> Supported Features Descriptions </b></p>";
		var numOfFeatures = card.features.length;
		for(i=0; i < numOfFeatures; i++){
			featurelist = featurelist + "<li class='list-group-item'>" + card.features[i].name +"</li>";
			featureDecHtml = featureDecHtml + "<p class='list-group-item list-group-item-secondary col-12'><b>"+ card.features[i].name +":</b> " +card.features[i].description+"</p>"
