...
function dragElement(elmnt) {
	...
	var obj_SC = document.getElementById("Humor_1");
	var obj_NC = document.getElementById("Humor_2");
	var obj_C = document.getElementById("Humor_3");
	var obj_VC = document.getElementById("Humor_4");
...
function getCard(){
	$.get(origin+'/api/card', {SortingType:'Sensitive', Previous:'0'}, getCardCallback); 
}

function saveCardSort(datatype, stackName){
	$.post(origin+"/api/card",
	{
		CardName: datatype,
		SortedInto: stackName,
		Path: path.toString(),
		SortingType:'Sensitive'
	},
...
function getCardCallback(data, status){
	if(status == "success"){
		...
		if (card.name != "Done"){
			...
			card_description.innerHTML = '<img src="img/memes/'+card.name+'.jpeg" />';
...
function getPreviousCard(){
	$.get(origin+'/api/card', {SortingType:'Sensitive', Previous:'1'}, getPreviousCallback);
}
...
function undoSort(){
	$.post(origin+"/api/card/undo",{SortingType:'Sensitive'}, undoCallback);
}
